...
import { isSongInListOfSongs } from "./utils";
...
class MusicPlayer extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    const { isAuthenticated, playingSong, playerQueue } = this.props;
    const { isAuthenticatedNext, playingSongNext, playerQueueNext } = nextProps;

    return (
      isAuthenticated !== isAuthenticatedNext ||
      playingSong !== playingSongNext ||
      playerQueue !== playerQueueNext
    );
  }
  ...
  render() {
    ...
    // params.onAudioPlay = songInfo => {
    //   const newSong = this.props.playingSong;
    //   if (isSongInListOfSongs(newSong, this.props.playerQueue)) {
    //     this.props.unpauseSong();
    //   } else {
    //     //this.props.playSongWithoutChangingQueue(newSong);
    //   }
    // };
    ...
    params.onAudioListsChange = (currentPlayId, audioLists, audioInfo) => {
      if (audioLists !== params.audioLists) {
        ...
        setTimeout(() => {
          ...
          this.props.playGroupOfSongs(audioLists);
        }, 3000);
