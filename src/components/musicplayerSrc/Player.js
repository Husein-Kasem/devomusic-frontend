...
export default class Player extends PureComponent {
  initPlayId = "";
  audioListRemoveAnimateTime = 350;
  NETWORK_STATE = {
    NETWORK_EMPTY: 0,
    NETWORK_IDLE: 1,
    NETWORK_LOADING: 2,
  ...
  componentWillReceiveProps({ audioLists, playIndex }) {
    if (!arrayEqual(audioLists)(this.props.audioLists)) {
      ...
      console.log("current play index", currentPlayIndex);
      if (currentPlayIndex !== playIndex) {
        ...
        if (
          Array.isArray(this.state.audioLists) &&
          this.state.audioLists.length >= 1
        ) {
          console.log(
            `(!isNaN(playIndex) && playIndex) ${!isNaN(playIndex) && playIndex}`
          );
          if (!isNaN(playIndex) && playIndex) {
            this.audioListsPlay(this.state.audioLists[_playIndex].id, true);
          } else {
            this.audioListsPlay(this.state.audioLists[0].id, true);
          }
        }
      }
    }
